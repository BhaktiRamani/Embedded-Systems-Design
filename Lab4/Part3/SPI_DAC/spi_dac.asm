...
;	spi_dac.c:182: delay_ms(10);
	...
	pop	ar5
	pop	ar7
;	spi_dac.c:185: P1_1 = 0;  // Select DAC
;	assignBit
	clr	_P1_1
;	spi_dac.c:191: SPDAT = high_byte;
	mov	_SPDAT,r5
;	spi_dac.c:192: while (!(SPSTA & (1<<7))); 
00104$:
	mov	a,_SPSTA
	jnb	acc.7,00104$
...
00107$:
	mov	a,_SPSTA
	jnb	acc.7,00107$
;	spi_dac.c:202: P1_1 = 1;  // Deselect DAC
;	assignBit
	setb	_P1_1
;	spi_dac.c:208: P1_1 = 1;  // Deselect DAC
;	assignBit
	setb	_P1_1
;	spi_dac.c:212: printf("E\n\r");
	mov	a,#___str_3
	push	acc
	mov	a,#(___str_3 >> 8)
	push	acc
...
;------------------------------------------------------------
;	spi_dac.c:220: void timer0_init(void)
;	-----------------------------------------
;	 function timer0_init
;	-----------------------------------------
_timer0_init:
;	spi_dac.c:222: TMOD &= 0xF0;    // Clear Timer0 mode bits
	anl	_TMOD,#0xf0
;	spi_dac.c:223: TMOD |= TIMER0_MODE1;  // Set Timer0 mode 1 (16-bit)
	orl	_TMOD,#0x01
;	spi_dac.c:226: TH0 = TH0_RELOAD;
	mov	_TH0,#0xfc
;	spi_dac.c:227: TL0 = TL0_RELOAD;
	mov	_TL0,#0x66
;	spi_dac.c:229: ET0 = 1;         // Enable Timer0 interrupt
;	assignBit
	setb	_ET0
;	spi_dac.c:230: TR0 = 1;         // Start Timer0
;	assignBit
	setb	_TR0
;	spi_dac.c:231: }
	ret
...
;------------------------------------------------------------
;	spi_dac.c:234: void timer0_isr(void) __interrupt 1
;	-----------------------------------------
;	 function timer0_isr
;	-----------------------------------------
_timer0_isr:
	...
	push	psw
	mov	psw,#0x00
;	spi_dac.c:237: TH0 = 0x4B;
	mov	_TH0,#0x4b
;	spi_dac.c:238: TL0 = 0x1C;
	mov	_TL0,#0x1c
;	spi_dac.c:240: ms_flag = 1;     // Set 1ms flag
	mov	dptr,#_ms_flag
	mov	a,#0x01
	movx	@dptr,a
;	spi_dac.c:242: printf("T \n\r");
	mov	a,#___str_5
	push	acc
	mov	a,#(___str_5 >> 8)
	push	acc
...
;------------------------------------------------------------
;	spi_dac.c:253: void spi_isr(void) __interrupt 9
;	-----------------------------------------
;	 function spi_isr
;	-----------------------------------------
_spi_isr:
	...
	push	psw
	mov	psw,#0x00
;	spi_dac.c:255: if(SPSTA == 0x80)  // Check for successful transmission
	mov	a,#0x80
	cjne	a,_SPSTA,00103$
;	spi_dac.c:257: transmission_complete = 1;
...
;------------------------------------------------------------
;	spi_dac.c:273: void spi_init(void) 
;	-----------------------------------------
;	 function spi_init
;	-----------------------------------------
_spi_init:
;	spi_dac.c:294: SPCON |= 0x10;
	orl	_SPCON,#0x10
;	spi_dac.c:295: SPCON |= 0x20;
	orl	_SPCON,#0x20
;	spi_dac.c:296: SPCON |= 0x40;
	orl	_SPCON,#0x40
;	spi_dac.c:303: }
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	spi_dac.c:310: void spi_transmission_start(void)
;	-----------------------------------------
;	 function spi_transmission_start
;	-----------------------------------------
_spi_transmission_start:
;	spi_dac.c:312: SPCON |= SPI_ENABLE;           // Enable SPI
	orl	_SPCON,#0x40
;	spi_dac.c:313: P1_1 = 0;
;	assignBit
	clr	_P1_1
;	spi_dac.c:314: }
	ret
...
;------------------------------------------------------------
;	spi_dac.c:321: void spi_transmission_stop(void) 
;	-----------------------------------------
;	 function spi_transmission_stop
;	-----------------------------------------
_spi_transmission_stop:
;	spi_dac.c:323: SPCON &= ~SPI_ENABLE;          // Disable SPI
	anl	_SPCON,#0xbf
;	spi_dac.c:324: }
	ret
	.area CSEG    (CODE)
