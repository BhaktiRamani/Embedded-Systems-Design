...
MAIN:
    ...
    MOV R0, #80H  ; Initialize ISR debug counter
    MOV R1, #00H ; Initialize main loop debug counter
...
MAIN_LOOP:
    ...
    INC R1   ; Increment main loop counter
    MOV A, R1
    CJNE A, #80H, SKIP_MAIN_RESET
    MOV R1, #00H  ; Reset to 00h if reached 80h
SKIP_MAIN_RESET:
...
TIMER0_ISR:
    ...
    MOV A, R0
    MOVX @DPTR, A      ; Write to debug latch
    INC R0    ; Increment ISR counter
    MOV A, R0
    CJNE A, #00H, SKIP_ISR_RESET
    MOV R0, #80H  ; Reset to 80h if reached 00h
SKIP_ISR_RESET:
