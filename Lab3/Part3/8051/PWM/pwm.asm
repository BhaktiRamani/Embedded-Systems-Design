...
;	pwm.c:22: SBUF = charToSend;  // Send character to serial buffer
	...
	mov	r7,a
	mov	_SBUF,r6
;	pwm.c:23: while (!TI);        // Wait for transmission to complete
00101$:
;	pwm.c:24: TI = 0;            // Clear transmission interrupt flag
;	assignBit
	jbc	_TI,00114$
	sjmp	00101$
00114$:
;	pwm.c:25: return charToSend;
	mov	dpl,r6
	mov	dph,r7
;	pwm.c:26: }
	ret
...
;	-----------------------------------------
_getchar:
;	pwm.c:34: while (!RI);        // Wait for reception to complete
00101$:
;	pwm.c:35: RI = 0;            // Clear reception interrupt flag
;	assignBit
	jbc	_RI,00114$
	sjmp	00101$
00114$:
;	pwm.c:36: return SBUF;       // Return received character
	mov	r6,_SBUF
	mov	r7,#0x00
	mov	dpl,r6
	mov	dph,r7
...
;	pwm.c:47: printf(" S - STOP PWM\n\r");
	...
	dec	sp
	dec	sp
;	pwm.c:48: printf(" F - MAX FREQUENCY MODE\n\r");
	mov	a,#___str_4
	push	acc
	mov	a,#(___str_4 >> 8)
	push	acc
	...
	dec	sp
	dec	sp
;	pwm.c:49: printf(" M - MIN FREQUENCY MODE\n\r");
	mov	a,#___str_5
	push	acc
	mov	a,#(___str_5 >> 8)
	push	acc
...
;	pwm.c:50: printf(" I - IDLE MODE\n\r");
	...
	dec	sp
	dec	sp
;	pwm.c:51: printf(" P - POWER DOWN MODE\n\r");
	mov	a,#___str_7
	push	acc
	mov	a,#(___str_7 >> 8)
	push	acc
	...
	dec	sp
	dec	sp
;	pwm.c:52: printf(" H - HIGH SPEED MODE ON\n\r");
	mov	a,#___str_8
	push	acc
	mov	a,#(___str_8 >> 8)
	push	acc
	...
	dec	sp
	dec	sp
;	pwm.c:53: printf(" Q - HIGH SPEED MODE OFF\n\r");
	mov	a,#___str_9
	push	acc
	mov	a,#(___str_9 >> 8)
	push	acc
...
;	pwm.c:87: case 'H':
	sjmp	00112$
00107$:
;	pwm.c:88: HS0_mode_on();
	lcall	_HS0_mode_on
;	pwm.c:89: break;
;	pwm.c:90: case 'Q':
	sjmp	00112$
00108$:
;	pwm.c:91: HS0_mode_off();
	lcall	_HS0_mode_off
;	pwm.c:92: default:
00109$:
;	pwm.c:93: printf("Invalid Command\n\r");
	mov	a,#___str_11
	push	acc
	mov	a,#(___str_11 >> 8)
	push	acc
...
;------------------------------------------------------------
;	pwm.c:105: void idle_mode()
;	-----------------------------------------
;	 function idle_mode
;	-----------------------------------------
_idle_mode:
;	pwm.c:107: printf("IDLE MODE\n\r");
	mov	a,#___str_12
	push	acc
	mov	a,#(___str_12 >> 8)
	push	acc
...
;------------------------------------------------------------
;	pwm.c:111: void power_down_mode()
;	-----------------------------------------
;	 function power_down_mode
;	-----------------------------------------
_power_down_mode:
;	pwm.c:113: printf("POWER DOWN MODE\n\r");
	mov	a,#___str_13
	push	acc
	mov	a,#(___str_13 >> 8)
	push	acc
...
;------------------------------------------------------------
;	pwm.c:116: void freq_max()
;	-----------------------------------------
;	 function freq_max
;	-----------------------------------------
_freq_max:
;	pwm.c:118: printf("MAXIMUM FREQUENCY\n\r");
	mov	a,#___str_14
	push	acc
	mov	a,#(___str_14 >> 8)
	push	acc
...
;------------------------------------------------------------
;	pwm.c:122: void freq_min()
;	-----------------------------------------
;	 function freq_min
;	-----------------------------------------
_freq_min:
;	pwm.c:124: printf("MINIMUM FREQUENCY\n\r");
	mov	a,#___str_15
	push	acc
	mov	a,#(___str_15 >> 8)
	push	acc
...
;------------------------------------------------------------
;	pwm.c:128: void pwm_init()
;	-----------------------------------------
;	 function pwm_init
;	-----------------------------------------
_pwm_init:
;	pwm.c:130: CCAPM0 = 0x00;
	mov	_CCAPM0,#0x00
;	pwm.c:131: CMOD = 0X82;
	mov	_CMOD,#0x82
;	pwm.c:132: CL = 0X00;
...
;	pwm.c:133: CH = 0X00;
	mov	_CH,#0x00
;	pwm.c:135: CCAP0L = 0XAC;
	mov	_CCAP0L,#0xac
;	pwm.c:136: CCAP0H = 0XAC;
	mov	_CCAP0H,#0xac
;	pwm.c:137: CCAPM0 = 0X42;
	mov	_CCAPM0,#0x42
;	pwm.c:138: }
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	pwm.c:140: void pwm_start()
;	-----------------------------------------
;	 function pwm_start
;	-----------------------------------------
_pwm_start:
;	pwm.c:142: printf("PWM START\n\r");
	mov	a,#___str_16
	push	acc
	mov	a,#(___str_16 >> 8)
	push	acc
...
;------------------------------------------------------------
;	pwm.c:146: void pwm_stop()
;	-----------------------------------------
;	 function pwm_stop
;	-----------------------------------------
_pwm_stop:
;	pwm.c:148: printf("PWM STOP\n\r");
	mov	a,#___str_17
	push	acc
	mov	a,#(___str_17 >> 8)
	push	acc
...
;	pwm.c:155: CCON |= 0x4C;
	orl	_CCON,#0x4c
;	pwm.c:156: CMOD |= 0x02;     // Fclk/2 freq set for PCA mode
	orl	_CMOD,#0x02
;	pwm.c:157: CCAPM0 = 0x4C;   // Compare mode + HSO, no interrupt
	mov	_CCAPM0,#0x4c
;	pwm.c:158: CL = 0x00;       // Clear PCA counter
	mov	_CL,#0x00
;	pwm.c:159: CH = 0x00;
	mov	_CH,#0x00
;	pwm.c:162: CCAP0L = 0xAC;   // Load compare low byte
	mov	_CCAP0L,#0xac
;	pwm.c:163: CCAP0H = 0xAC;   // Load compare high byte
	mov	_CCAP0H,#0xac
;	pwm.c:164: }
	ret
;------------------------------------------------------------
...
;	-----------------------------------------
_HS0_mode_off:
;	pwm.c:168: printf("HIGH SPEED MODE OFF\n\r");
	mov	a,#___str_19
	push	acc
	mov	a,#(___str_19 >> 8)
	push	acc
